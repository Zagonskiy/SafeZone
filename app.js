...
// ==========================================
// === СИСТЕМА ФРАГМЕНТАЦИИ (CHUNKING) ===
// ==========================================
// Разбиваем файл на куски по 500KB (чтобы влезло в документ с запасом)
const CHUNK_SIZE = 500 * 1024; 

async function uploadFileInChunks(file, parentDocId) {
    const totalChunks = Math.ceil(file.size / CHUNK_SIZE);
    
    // Используем Batch (пакетную запись) для надежности
...
async function loadVideoFromChunks(msgId, mimeType) {
    ...
    // Собираем данные
    let combinedData = []; // Для Blob
    
    // Проблема Base64: Нужно убрать префикс data:..., чтобы склеить
    // Но FileReader readAsDataURL дает префикс для каждого куска.
    // Упрощенный вариант: Мы просто склеим Base64 строки? Нет, это сломает кодировку.
    // Правильный вариант: Конвертировать Base64 обратно в бинарный код.
    
    // ДЛЯ ПРОСТОТЫ: Мы сохраняем чанки просто как строки Base64. 
    // При чтении мы берем контент после запятой (base64 тело), склеиваем, и конвертируем.
    
    // Перепишем uploadFileInChunks на более простой вариант для видео?
    // Нет, будем использовать fetch для конвертации base64 -> blob
    
    let parts = [];
    snap.forEach(d => {
        const base64Part = d.data().data;
        // fetch умеет кушать data-uri
...
btnConfirmPhoto.addEventListener('click', async () => {
    ...
    btnConfirmPhoto.innerText = isVideo ? "ЗАГРУЗКА (ЖДИТЕ)..." : "СЖАТИЕ...";
    ...
    try {
        ...
        if (isVideo) {
            // ДЛЯ ВИДЕО: Создаем сообщение-пустышку, потом грузим чанки
            isChunked = true;
        } else {
            // ДЛЯ ФОТО: Сжимаем и шлем как раньше
        ...
        const msgData = {
            ...
            mimeType: selectedFile.type // Сохраняем тип для плеера
        };

        if (!isChunked) {
            msgData.imageBase64 = contentBase64;
        } else {
            // Флаг, что видео загружено кусками
        ...
        // 2. Если это видео - начинаем загрузку кусков
        if (isChunked) {
            await uploadFileInChunks(selectedFile, msgRef.id);
            // Обновляем сообщение, что загрузка завершена (опционально)
        }
...
// ==========================================
// === РЕНДЕР (ОБНОВЛЕН ДЛЯ ЗАГРУЗКИ ВИДЕО) ===
// ==========================================
        ...
        loadBtn.onclick = async () => {
            ...
            try {
                ...
                if (videoBlob) {
                    ...
                    // Клик на превью открывает лайтбокс
                    vid.onclick = () => viewMedia('video', vidUrl, msg.text);
                    
                    videoContainer.innerHTML = ''; // Убираем кнопку
...
// ОСТАЛЬНОЕ (БЕЗ ИЗМЕНЕНИЙ, НО НУЖНО ДЛЯ РАБОТЫ)
if (msgInput) {
    msgInput.addEventListener('input', () => {
        const text = msgInput.value.trim();
        if (text.length > 0) { btnSendText.style.display = 'flex'; btnMicRec.style.display = 'none'; } 
        else { btnSendText.style.display = 'none'; btnMicRec.style.display = 'flex'; }
    });
}

// ЗАПИСЬ АУДИО (СОХРАНЯЕМ ТВОЮ ГИБРИДНУЮ ЛОГИКУ)
        ...
        mediaRecorder.onstop = async () => {
            ...
            reader.onloadend = async () => {
                ...
                try {
                    ...
                    await updateDoc(doc(db, "chats", currentChatId), { lastUpdated: serverTimestamp(), hiddenFor: [] });
                } catch (e) { console.error(e); showModal("СБОЙ ОТПРАВКИ", "alert"); }
            };
        };
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (!currentUserData || currentUserData.uid !== user.uid) { const snap = await getDoc(doc(db, "users", user.uid)); if (snap.exists()) currentUserData = { uid: user.uid, ...snap.data() }; }
        updateMyDisplay(); loadMyChats();
    } else { appInterface.classList.add('hidden'); authScreen.classList.add('active'); currentUserData = null; }
});
document.getElementById('msg-form').addEventListener('submit', async (e) => {
    e.preventDefault(); const text = msgInput.value.trim(); if (!text || !currentChatId) return;
    await addDoc(collection(db, "chats", currentChatId, "messages"), {
        text, senderId: auth.currentUser.uid, senderNick: currentUserData.nickname, senderAvatar: currentUserData.avatarBase64 || null, createdAt: serverTimestamp(), edited: false, read: false
    });
    await updateDoc(doc(db, "chats", currentChatId), { lastUpdated: serverTimestamp(), hiddenFor: [] });
    msgInput.value = ''; btnSendText.style.display = 'none'; btnMicRec.style.display = 'flex';
});
window.confirmDeleteChat = (chatId) => { chatToDeleteId = chatId; deleteChatModal.classList.add('active'); };
...
document.getElementById('btn-del-me').addEventListener('click', async () => {
    if (!chatToDeleteId) return; const btn = document.getElementById('btn-del-me'); btn.disabled = true; btn.innerText = "УДАЛЕНИЕ...";
    try { await updateDoc(doc(db, "chats", chatToDeleteId), { hiddenFor: arrayUnion(auth.currentUser.uid), [`clearedAt.${auth.currentUser.uid}`]: serverTimestamp() });
        deleteChatModal.classList.remove('active'); if (currentChatId === chatToDeleteId) document.getElementById('back-btn').click(); chatToDeleteId = null;
...
document.getElementById('btn-del-all').addEventListener('click', async () => {
    if (!chatToDeleteId) return; if (!confirm("ВЫ УВЕРЕНЫ?")) return;
    try { const msgsQ = query(collection(db, "chats", chatToDeleteId, "messages")); const msgsSnap = await getDocs(msgsQ); const deletePromises = msgsSnap.docs.map(d => deleteDoc(d.ref)); await Promise.all(deletePromises); await deleteDoc(doc(db, "chats", chatToDeleteId)); if (currentChatId === chatToDeleteId) document.getElementById('back-btn').click(); deleteChatModal.classList.remove('active'); } catch (e) { alert("ОШИБКА"); }
});
window.deleteMsg = async (cId, mId) => { if (await showModal('УДАЛИТЬ?', 'confirm')) await deleteDoc(doc(db, "chats", cId, "messages", mId)); };
window.editMsg = async (cId, mId, old) => { const val = await showModal('ИЗМЕНИТЬ:', 'prompt', old); if (val && val !== old) await updateDoc(doc(db, "chats", cId, "messages", mId), { text: val, edited: true }); };
// Поиск и профиль (стандартный код)
if (searchInput) { searchInput.addEventListener('input', (e) => { const text = e.target.value.trim(); if (!text) { searchResultsArea.style.display = 'none'; if(searchIndicator) searchIndicator.classList.remove('active'); return; } if(searchIndicator) searchIndicator.classList.add('active'); searchResultsArea.style.display = 'block'; searchList.innerHTML = '<div style="padding:15px; opacity:0.7;">>> СКАНИРОВАНИЕ...</div>'; clearTimeout(searchTimeout); searchTimeout = setTimeout(() => executeSearch(text), 500); }); }
async function executeSearch(queryText) { try { const endText = queryText + '\uf8ff'; const q = query(collection(db, "users"), orderBy("nickname"), where("nickname", ">=", queryText), where("nickname", "<=", endText), limit(3)); const snap = await getDocs(q); renderSearchResults(snap); } catch (error) { searchList.innerHTML = "ОШИБКА"; } finally { if(searchIndicator) searchIndicator.classList.remove('active'); } }
function renderSearchResults(snapshot) { searchList.innerHTML = ''; if (snapshot.empty) { searchList.innerHTML = "НЕТ ЦЕЛИ"; return; } snapshot.forEach(docSnap => { const user = docSnap.data(); const uid = docSnap.id; if (uid === auth.currentUser.uid) return; const item = document.createElement('div'); item.className = 'search-item'; const avatarSrc = user.avatarBase64 || DEFAULT_AVATAR; const avatarHTML = `<img src="${avatarSrc}" style="width:25px; height:25px; border-radius:50%; margin-right:8px;">`; item.innerHTML = `<span>${avatarHTML}${user.nickname}</span>`; item.onclick = () => { searchInput.value = ''; searchResultsArea.style.display = 'none'; startChat(uid, user.nickname); }; searchList.appendChild(item); }); }
async function startChat(targetUid, targetNick) { const chatDocId = [auth.currentUser.uid, targetUid].sort().join("_"); await setDoc(doc(db, "chats", chatDocId), { participants: [auth.currentUser.uid, targetUid], participantNames: [currentUserData.nickname, targetNick], lastUpdated: serverTimestamp() }, { merge: true }); openChat(chatDocId, targetNick); }
