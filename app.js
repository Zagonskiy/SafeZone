...
function generateVideoThumbnail(file) {
    return new Promise((resolve) => {
        ...
        video.onloadeddata = () => {
            // Если видео короче 1 сек, берем начало
        ...
        video.onseeked = () => {
            ...
            // Сжимаем обложку очень сильно, чтобы не забивать базу
            resolve(canvas.toDataURL('image/jpeg', 0.5));
        };
        
        video.onerror = () => {
            resolve(null); // Если ошибка, вернем null
        };
    });
}
...
btnConfirmPhoto.addEventListener('click', async () => {
    ...
    try {
        ...
        if (isVideo) {
            isChunked = true;
            // Генерируем обложку
            videoThumb = await generateVideoThumbnail(selectedFile);
        } else {
...
function viewMedia(type, src, caption) {
    if (type === 'video') {
        ...
        // Автоплей для удобства
        fullVideoView.play().catch(e => console.log("Autoplay blocked"));
    } else {
...
function renderMessage(docSnap) {
    ...
    if (msg.audioBase64) {
        ...
        // Используем обложку или заглушку
        ...
        // Клик -> Скачивание -> Просмотр
        videoContainer.onclick = async () => {
            // Если уже загружено локально (во избежание повторной загрузки при клике)
            ...
            try {
                ...
                if (videoBlob) {
                    ...
                    videoContainer.dataset.blobUrl = vidUrl; // Сохраняем URL
                    viewMedia('video', vidUrl, msg.text);
                    playIcon.style.border = "2px solid var(--text-green)"; // Вернуть как было
                } else {
                    alert("ОШИБКА ВИДЕО");
                }
            } catch (e) {
